# Define file paths
real_file = "./number_of_files_in_download_directory_combined.txt"
...
def main():
    ...
    for key, expected_value in expected_data.items():
        ...
        if real_value is None:
            # Entire entry is missing, count all expected elements
            total_missing += expected_value
            missing_or_mismatched.append(f"{key}: {expected_value} missing")
        elif real_value != expected_value:
            # Calculate the difference
            difference = expected_value - real_value
            total_missing += difference
            missing_or_mismatched.append(f"{key}: {difference} missing")
